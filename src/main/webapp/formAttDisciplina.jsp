...
<title>Nextcid | Atualizar Disciplina</title>
</head>
<!-- id = # / class = . -->
        ...
        <div class="row">
          <div class="col-sm-8">
            <div class="card" style="height: auto;">
              <div class="card-body">
                <h4 class="card-title">Atualização de Disciplina</h4>
                <jsp:useBean id="disciplina" scope="session" class="br.edu.unicid.bean.Disciplina" />
                <form action="ServletDisciplina?cmd=atualizar" method="post">                     
                  <div class="form-group">
                    <label>Codigo:</label>
                    <input type="text" class="form-control" name="idDisciplina" value="<%= disciplina.getIdDisciplina() %>" readonly="readonly"><br>
                    <label>Nome da Disciplina:</label>
                    <input type="text" class="form-control" required name="nomeDisciplina" value="<%= disciplina.getNomeDisciplina() %>"><br>
                    <label>Curso Relacionado:</label><br>
                        <select name="idCurso" class="form-control" value="<%= disciplina.getIdCurso() %>" aria-label="Default select example">
                           <option value="<%= disciplina.getIdCurso() %>" selected><%= disciplina.getNomeCurso() %></option>
                            <%
                                CursoDao daocurso = new CursoDao();
    							List <Curso> listacurso = new ArrayList<Curso>();
    							listacurso = (ArrayList)daocurso.todosCursos();
    							for(Curso a: listacurso){%>
                                <%if (a.getIdCurso() == disciplina.getIdCurso()){
                    ...
                    <br>						
                  </div>                                                                                            <button type="submit" class="btn btn-outline-primary">Atualizar</button>        
                </form> 
              </div>
            </div>          
          </div>          
